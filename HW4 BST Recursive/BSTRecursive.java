...
public class BSTRecursive {
	...
	private int numLeaves(BSTNode n) {
		...
		if(n.left == null) return numLeaves(n.right);
		if(n.right == null) return numLeaves(n.left);
	...
	private BSTRecursive greaterThan(BSTNode n, int v, BSTRecursive result) {
		if(n != null) {
			if(n.data > v) result.insert(n.data);
	...
	private String toStringInOrder(BSTNode n) {
		if(n == null) return "";
		return (toStringInOrder(n.left) + " " +  n.data + " " +toStringInOrder(n.right)).trim();
	...
	private boolean isBST(BSTNode n) {
		...
		return allLessThan(n.left, n) & allGreaterThan(n.right, n) && isBST(n.left) && isBST(n.right);
	}
	
	private boolean isBSTHelper(BSTNode n, BSTNode nr) {
		//if we do not yet reach the leaf
		if(nr == null) return true;
		if(nr.right == null && nr.left == null) return allLessThan(nr,n) && allGreaterThan(nr,n);
		if(nr.right == null) return isBSTHelper(n, nr.left);
		if(nr.left == null) return isBSTHelper(n,nr.right);
		return isBSTHelper(n,nr.right) && isBSTHelper(n,nr.left);
	}
	...
	private BSTNode findParentForwardDirection(BSTNode n, BSTNode d, BSTNode parent) {
		if(n != null) {
			if(n == d) return n.parent;
			if(findParentForwardDirection(n.left,d,parent) != null) return findParentForwardDirection(n.left,d,parent);
			if(findParentForwardDirection(n.right,d,parent) != null) return findParentForwardDirection(n.right,d,parent);
		}
		return null;
	}
